...
- Stage 30의 두 Boss는 자연스러운 부분 겹침을 허용한다. 중심 거리가 평균 Boss 크기의 `55%`보다 가까운 심한 겹침에서만 프레임당 최대 `1.5px`의 작은 위치 보정과 서로 반대인 속도 성분을 적용해 부드럽게 갈라진다. 겹친 터치 영역에서는 터치 지점과 중심이 더 가까운 Boss를 선택하며, 타격 가속은 유지하되 이동 속도는 최대 `220px/s`로 제한한다.
