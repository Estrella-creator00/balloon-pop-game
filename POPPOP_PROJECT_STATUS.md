...
| `C-01` | 코인 충전 | 무료출시 플래그로 홈 진입점과 화면 비활성화. 향후 결제 계약 코드는 보존 |
| `SET-01` | 설정 메인 | 구현됨 |
...
- 무료출시 빌드에서는 홈의 현금 코인팩 `+` 버튼과 C-01 진입 경로를 표시하지 않는다.
- 코인 패키지는 `lib/coin/coin_package.dart`의 `CoinPackage`와 `coinPackages`에서 한 곳에 정의한다. 현재 UI 상품은 300/700/1,500/3,500 코인 4개다.
- `lib/config/release_features.dart`의 컴파일 타임 플래그 `cashCoinPurchasesEnabled`는 `false`이며 원격·사용자 설정으로 바꿀 수 없다. 이 상태에서는 플랫폼 결제 gateway 생성, purchase stream 구독, 상품 조회, 구매·복원·영수증 전송 API 호출이 모두 실행되지 않는다.
- 향후 결제 도입을 위한 화면·서비스·검증 계약은 플래그 뒤에 보존하지만 현재 출시 앱에서는 접근할 수 없다.
...
- 향후 사업자등록·스토어 상품·서버 영수증 검증 준비 후 무료출시 플래그를 해제하는 실제 결제 도입
- 정식 이용약관 본문
- 정식 개인정보처리방침 본문
- 실제 문의 채널
