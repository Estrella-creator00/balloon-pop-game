...
Future<void> clearCurrentLegendaryCanvasStage(WidgetTester tester) async {
  final canvasFinder = find.byKey(
    const ValueKey('phase-1-persistent-game-canvas'),
  );
  var pointer = 16000;
  for (var attempt = 0; attempt < 200; attempt++) {
    final canvas = tester.widget<PersistentGameCanvas<Balloon>>(canvasFinder);
    final normalBalloons = canvas.renderState.basicBalloons
        .where((balloon) => !balloon.isFake)
        .toList(growable: false);
    if (normalBalloons.isEmpty) return;
    final target = normalBalloons.last;
    canvas.onPointerDown(
      PointerDownEvent(
        pointer: pointer++,
        kind: ui.PointerDeviceKind.touch,
  ...
  throw TestFailure('Legendary Canvas stage did not clear within 200 hits.');
}
...
void main() {
  ...
  test('gemi shard selects a pre-rendered sprite for each catalog color', () {
    final gemi = BalloonSkinCatalog.byIdOrDefault('balloon-lumen');
    expect(gemi.runtimeShardAssetPaths, hasLength(4));
    for (final color in gemi.colorPalette) {
      final path = gemiShardAssetForColor(gemi, color);
      expect(path, gemi.runtimeShardAssetPaths[color.toARGB32()]);
      expect(path, endsWith('_runtime.png'));
    }
  });
  ...
  testWidgets('every balloon definition uses the shared fake renderer path', (
    ...
    for (final definition in BalloonSkinCatalog.definitions) {
      ...
      final usesPrecomposedFake = definition.runtimeFakeColorAssetPaths
          .containsKey(definition.previewColor.toARGB32());
      expect(
      ...
      switch (definition.rendererType) {
        ...
        case BalloonRendererType.image:
          ...
          expect(filtered, usesPrecomposedFake ? findsNothing : findsWidgets);
  ...
  testWidgets('gemi catalog colors use plain pre-rendered body images', (
    tester,
  ) async {
    final gemi = BalloonSkinCatalog.byIdOrDefault('balloon-lumen');
    final color = gemi.colorPalette.first;
    await tester.pumpWidget(
      MaterialApp(
        home: Row(
          children: [
    ...
    for (final image in tester.widgetList<Image>(find.byType(Image))) {
      expect(
        ...
        gemi.runtimeColorAssetPaths[color.toARGB32()],
      );
    }
  });

  testWidgets('fake gemi also uses a precomposed plain image', (tester) async {
    final gemi = BalloonSkinCatalog.byIdOrDefault('balloon-lumen');
    final color = gemi.colorPalette.last;
    await tester.pumpWidget(
      MaterialApp(
    ...
    expect(
      assetNameOf(tester.widget<Image>(find.byType(Image)).image),
      gemi.runtimeFakeColorAssetPaths[color.toARGB32()],
    );
  });
  ...
  testWidgets('crystal pulse only changes a cached glow opacity', (
    ...
    final backgroundImage = tester.widget<Image>(
      find.descendant(
        of: find.byKey(const ValueKey('background-crystal-image-boundary')),
        matching: find.byType(Image),
      ),
    );
    final pulseOpacity = find.ancestor(
      of: find.byKey(
        const ValueKey('background-crystal-impact-glow-image'),
      ),
      matching: find.byType(Opacity),
    );
    expect(tester.widget<Opacity>(pulseOpacity).opacity, 0.18);
    ...
    expect(
      identical(
        ...
        tester.widget<Image>(
          find.descendant(
            of: find.byKey(
              const ValueKey('background-crystal-image-boundary'),
            ),
            matching: find.byType(Image),
          ),
        ),
  ...
  test('shushu cream groups keep burst wall and front repaint bounds local',
      () {
    AssetVisualEffect effect(Offset center, AssetEffectPaintLayer layer) =>
        AssetVisualEffect(
          assetPath: 'cream.png',
    ...
    final effects = <AssetVisualEffect>[
      effect(const Offset(195, 350), AssetEffectPaintLayer.shushuBurst),
      effect(const Offset(4, 220), AssetEffectPaintLayer.shushuWallLeft),
      effect(const Offset(386, 460), AssetEffectPaintLayer.shushuWallRight),
      effect(const Offset(260, 180), AssetEffectPaintLayer.shushuFront),
    ];
    for (final layer in const [
      AssetEffectPaintLayer.shushuBurst,
      AssetEffectPaintLayer.shushuWallLeft,
      AssetEffectPaintLayer.shushuWallRight,
      AssetEffectPaintLayer.shushuFront,
    ]) {
      final bounds = assetEffectPaintBounds(effects, viewport, layer);
      expect(bounds.width, lessThan(80));
      expect(bounds.height, lessThan(80));
    }
  });
  ...
  test(
    ...
    () {
      ...
      expect(
        ...
        'assets/images/gemi_pickaxe_glow_runtime.png',
      );
    ...
    for (final size in <Size>[
      ...
      expect(
        tester
            .getSize(
              find.byKey(
                const ValueKey('stage-card-action-visual-21 ~ 30'),
              ),
            )
            .height,
        36,
  ...
  testWidgets('shop opens the balloon product list directly', (tester) async {
    ...
    expect(defaultRenderer, findsOneWidget);
    expect(tester.getSize(defaultRenderer), const Size(102, 128));
    final heartRenderer = find.descendant(
      of: find.byKey(const ValueKey('store-product-balloon-heart')),
      matching: find.byType(BalloonSkinRenderer),
    );
    expect(tester.getSize(heartRenderer), const Size(48, 56));
    for (final id in <String>['balloon-star', 'balloon-flower']) {
      final renderer = find.descendant(
        of: find.byKey(ValueKey('store-product-$id')),
        matching: find.byType(BalloonSkinRenderer),
      );
      expect(tester.getSize(renderer), const Size(48, 56));
    }
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        tester.getSize(
          find.byKey(const ValueKey('balloon-preview-default-uniform')),
        ),
        const Size(150, 176),
      );
      expect(
          tester.getSize(find.byWidget(previewRenderer)), const Size(102, 128));
      expect(
        tester
  ...
  testWidgets('gemi preview paints eight color-tinted official shards', (
    ...
    final previewShard =
        gemi.runtimeShardAssetPaths[gemi.previewColor.toARGB32()];
    expect(canvas.effectsForAsset(previewShard!), 8);
    expect(canvas.preloadAssets[previewShard], 128);
  ...
  testWidgets('legendary tools apply damage only at their impact frame', (
    ...
    for (final id in const ['balloon-lumen', 'balloon-chouchou']) {
      ...
      if (id == 'balloon-lumen') {
        ...
        final canvas = tester.widget<AssetEffectsCanvas>(effectLayer);
        expect(canvas.toolVisuals, hasLength(1));
        expect(
          canvas.toolVisuals.single.assetPath,
          'assets/images/gemi_pickaxe_glow_runtime.png',
        );
      ...
      if (id == 'balloon-lumen') {
        ...
        final gemi = BalloonSkinCatalog.byIdOrDefault('balloon-lumen');
        final shardCount = gemi.runtimeShardAssetPaths.values.fold<int>(
          0,
          (sum, path) => sum + assetCanvas.effectsForAsset(path),
        );
        expect(shardCount, greaterThanOrEqualTo(8));
  ...
  test('phase 4B adds only GEMI and SHUSHU to the phase 4A Canvas scope', () {
    ...
    expect(
      phase4BCanvasSkinIds.difference(phase4ACanvasSkinIds),
      const <String>{'balloon-lumen', 'balloon-chouchou'},
    );
    expect(
      gameplayCanvasStageEnabled(
  ...
  test('phase 4B render views keep GEMI runtime bodies and SHUSHU breathe', () {
    final gemi = BalloonSkinCatalog.byIdOrDefault('balloon-lumen');
    final gemiColor = gemi.colorPalette.first;
    final normalGemi = Balloon(
      id: 7101,
      position: Offset.zero,
      velocity: Offset.zero,
      color: gemiColor,
    ...
    final shushu = Balloon(
      id: 7103,
      position: Offset.zero,
      velocity: Offset.zero,
      color: const Color(0xFFD99542),
      size: 80,
      floatPhase: pi / 2,
      floatPower: 0,
      hp: 2,
      maxHp: 2,
      skinId: 'balloon-chouchou',
    );

    expect(
      normalGemi.spriteAssetPath,
      gemi.runtimeColorAssetPaths[gemiColor.toARGB32()],
    );
    expect(normalGemi.spriteColorMatrix, isNull);
    expect(
      fakeGemi.spriteAssetPath,
      gemi.runtimeFakeColorAssetPaths[gemiColor.toARGB32()],
    );
    expect(fakeGemi.spriteColorMatrix, isNull);
  ...
  testWidgets(
    'phase 4B renders legendary bodies on Canvas and preserves tool overlays',
    (tester) async {
      for (final skinId in const <String>[
        'balloon-lumen',
        'balloon-chouchou',
      ]) {
        equipBalloonSkinForTest(skinId);
        await tester.pumpWidget(
          const BalloonPopApp(
            gameplayRendererMode: GameplayRendererMode.canvasPhase4B,
        ...
        final balloon = canvas.renderState.basicBalloons.first;
        expect(find.byType(BalloonSkinRenderer), findsNothing);
        expect(balloon.spriteAssetPath, isNotNull);
        expect(
          canvas.spriteCache.isPrepared(balloon.spriteAssetPath!),
          isTrue,
        );
        ...
        await tester.pump();
        final overlay = tester.widget<AssetEffectsCanvas>(
          find.byKey(const ValueKey('asset-effects-boundary')),
        );
        expect(overlay.toolVisuals, hasLength(1));
        expect(overlay.preloadAssets, isNotEmpty);
  ...
  testWidgets(
    'phase 4B legendary 2-hit multitouch keeps Canvas and painter identity',
    (tester) async {
      ProgressStorage.unlockSecondSection();
      for (final skinId in const <String>[
        'balloon-lumen',
        ...
        final canvas =
            tester.widget<PersistentGameCanvas<Balloon>>(canvasFinder);
        final painter = tester.widget<CustomPaint>(painterFinder).painter;
        final targets = canvas.renderState.basicBalloons.take(2).toList();
        expect(targets, hasLength(2));
        expect(targets.every((balloon) => balloon.hp == 2), isTrue);
        final hitPoints = [
          for (final target in targets)
            exclusiveBalloonHitPoint(
              target,
              canvas.renderState.basicBalloons.where(
                (balloon) => balloon.id != target.id,
              ),
            ),
        ];
        expect(hitPoints, everyElement(isNotNull));
        ...
        for (var index = 0; index < targets.length; index++) {
          canvas.onPointerDown(
            PointerDownEvent(
              pointer: 7300 + index,
              kind: ui.PointerDeviceKind.touch,
              position: hitPoints[index]!,
            ),
          );
        }
        await tester.pump(const Duration(milliseconds: 80));

        expect(targets.every((balloon) => balloon.hp == 1), isTrue);
        expect(
          identical(
            tester.widget<PersistentGameCanvas<Balloon>>(canvasFinder),
            canvas,
        ...
        final duplicateTarget = targets.first;
        final duplicatePoint = await waitForExclusiveCanvasHitPoint(
          tester,
          canvasFinder,
          duplicateTarget.id,
        );
        for (final pointer in const <int>[7350, 7351]) {
          canvas.onPointerDown(
            PointerDownEvent(
              pointer: pointer,
              kind: ui.PointerDeviceKind.touch,
              position: duplicatePoint,
            ),
          );
        }
        await tester.pump(const Duration(milliseconds: 80));
        expect(
            canvas.renderState.basicBalloons, isNot(contains(duplicateTarget)));
        expect(canvas.renderState.basicBalloons, contains(targets.last));

        final finalPoint = await waitForExclusiveCanvasHitPoint(
          tester,
          canvasFinder,
          targets.last.id,
        );
  ...
  testWidgets('phase 4B legendary Fake balloons reuse Canvas hit logic', (
    tester,
  ) async {
    for (final skinId in const <String>[
      'balloon-lumen',
      ...
      final before = canvas.renderState.basicBalloons.length;
      final fakePoint = await waitForExclusiveCanvasHitPoint(
        tester,
        find.byKey(const ValueKey('phase-1-persistent-game-canvas')),
        fake.id,
      ...
      expect(canvas.renderState.basicBalloons, isNot(contains(fake)));
      expect(canvas.renderState.basicBalloons, hasLength(before - 1));
      expect(find.byType(BalloonSkinRenderer), findsNothing);
  ...
  testWidgets('phase 4B uses legendary sprites for Canvas bosses', (
    tester,
  ) async {
    for (final skinId in const <String>[
      'balloon-lumen',
      ...
      for (var stage = 1; stage <= 9; stage++) {
        await clearCurrentLegendaryCanvasStage(tester);
        expect(find.text('Stage Clear!'), findsOneWidget);
        await tester.pump(const Duration(milliseconds: 400));
      }
      ...
      expect(
        canvas.spriteCache.isPrepared(
          boss.spriteAssetPath!,
      ...
      canvas.onPointerDown(
        PointerDownEvent(
          pointer: 7500,
          kind: ui.PointerDeviceKind.touch,
          position: boss.position + Offset(boss.size / 2, boss.size / 2),
        ),
      );
      await tester.pump(const Duration(milliseconds: 40));
      expect(boss.hp, 9);
